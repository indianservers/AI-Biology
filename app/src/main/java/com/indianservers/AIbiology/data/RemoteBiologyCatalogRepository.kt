...
class RemoteBiologyCatalogRepository(
    ...
    private val cacheNamespace =
        namespace.lowercase().replace(Regex("[^a-z0-9._-]"), "_").ifBlank { DEFAULT_NAMESPACE }
    ...
    private val thumbnailDirectory =
        File(
            appContext.filesDir,
            if (cacheNamespace == DEFAULT_NAMESPACE) "biology/catalog/thumbnails"
            else "biology/catalog/$cacheNamespace/thumbnails"
        ).apply { mkdirs() }
    private val preferences =
        appContext.getSharedPreferences(
            if (cacheNamespace == DEFAULT_NAMESPACE) PREFERENCES_NAME
            else "${PREFERENCES_NAME}_$cacheNamespace",
    ...
    companion object {
        ...
        private const val DEFAULT_NAMESPACE = "biology"
