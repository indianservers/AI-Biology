...
import com.indianservers.biology.data.MicroscopySlide
import com.indianservers.biology.databinding.ItemMicroscopySlideBinding
import java.io.File
...
class MicroscopySlideAdapter(
    ...
    inner class SlideHolder(
        ...
        private fun loadThumbnail(slide: MicroscopySlide) {
            ...
            val expectedId = slide.id
            val available = thumbnailFile(slide)
            if (available != null) {
                showThumbnail(expectedId, available)
            } else {
                requestThumbnail(slide) { file ->
                    if (file != null) showThumbnail(expectedId, file)
                }
            }
        }
        ...
        private fun showThumbnail(expectedId: String, file: File) {
            ...
            if (getItem(bindingAdapterPosition).id != expectedId) return
